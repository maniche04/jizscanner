...
Font corbelf = new Font("Segoe UI",Font.PLAIN,13);
lst.setFont(corbelf);
this.getContentPane().add(new JScrollPane(lst), BorderLayout.CENTER);
setButtons();
//((JPanel)this.getContentPane()).setBorder(BorderFactory.createLineBorder(Color.lightGray, 3));
//lst.setToolTipText("Press Alt+ ↑, Alt+ ↓ to navigate, Insert key to select");
}
...
public void setActiveColumn(int col) { activeColumn = col; }
private void setButtons() {
//JButton b[] = {new JButton("Up (Alt+ ↑)"), new JButton("Down (Alt+ ↓)"), new JButton("Select(Insert)")};
//b[0].setMnemonic(KeyEvent.VK_DOWN);
//b[1].setMnemonic(KeyEvent.VK_DOWN);
//
//char c[] = {'U', 'D', 'S'};
//String txt[] = {"Move Up (Alt+ ↑)","Move Down (Alt+ ↓)","Select (Insert)"};
...
public void addItems(String it[]) { val.addAll(java.util.Arrays.asList(it)); }
public void addItems(java.util.List<String> init) { val.addAll(init); }
...
final InputMap im = tableParent.getInputMap(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
...
else parent.setText((String)lst.getSelectedValue());
this.setVisible(false);
}
private void tableSelection() {
int r = tableParent.getSelectedRow(), c = tableParent.getSelectedColumn();
try { tableParent.getCellEditor(r,c).stopCellEditing(); } catch(Exception ex) {}
lst.validate();
...
int index = lst.getSelectedIndex();
if(kc == KeyEvent.VK_UP || (parent instanceof JTextField && kc == KeyEvent.VK_UP))
    
moveUp();

else if(kc == KeyEvent.VK_DOWN || (parent instanceof JTextField && kc == KeyEvent.VK_DOWN))
...
else txt = parent.getText().toLowerCase();
tempVector.clear();
int index = lst.getSelectedIndex();
...
else p = tableParent.getLocationOnScreen(); 
